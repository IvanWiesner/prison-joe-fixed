...
function App() {
  ...
  // if (localStorage.cartItems) { notes new notes
  ...
  const addToCart = (merch) => {
    ...
    if (merchExist) {
      setCartItems(
        cartItems.map((item) =>
          item.id === merch.id
            ? { ...merchExist, quantity: merchExist.quantity + 1 }
            : item
        )
      );
      ...
      localStorage.setItem(
        'cartItems',
        JSON.stringify([...cartItems, { ...merch, quantity: 1 }])
      );
    }
  };
  ...
  return (
    <div className='App'>
      ...
      <Switch>
        ...
        <Route
          path='/checkout'
          component={() => <CheckoutPage merches={merches} />}
        />
        {login && (
          <Route
            path='/newmerch'
